...
static void freeListValue(struct key_value* entry)
{
    if (entry->type == LIST_MAP_VALUE_LIST || entry->type == LIST_MAP_VALUE_STREAM)
    {
        ...
        free(entry->data.list.items);#include "../headers/ListMap.h"
...
{
    if (collection == NULL || key == NULL) return -1;

    for (size_t i = 0; i < collection->count; ++i)
    {
        if (strcmp(collection->kvPairs[i].key, key) == 0)
        {
            if (collection->kvPairs[i].expires_at_ms != 0 && collection->kvPairs[i].expires_at_ms <= now_ms)
            {
                // Expired, treat as not found
                return -1;
            }
            if (collection->kvPairs[i].type != LIST_MAP_VALUE_STRING)
            {
                return -1;
            }
            // if key exists and doesnt have numerical value
            if (collection->kvPairs[i].data.value == NULL)
            {
                return -1;
            }
            char* endptr;
            int64_t current_value = strtoll(collection->kvPairs[i].data.value, &endptr, 10);
            if (*endptr != '\0') return -1; // Not a valid integer
            if (current_value == INT64_MAX) return -1; // Would overflow
            current_value++;
            size_t len = snprintf(NULL, 0, "%" PRId64, current_value) + 1;
            collection->kvPairs[i].data.value = realloc(collection->kvPairs[i].data.value, len);
            if (collection->kvPairs[i].data.value == NULL) return -1;
            snprintf(collection->kvPairs[i].data.value, len, "%" PRId64, current_value);
            return current_value;
        }
    }
    // Key not found, create if possible
    if (collection->count >= listMapCapacity) return -1;
    // Create new entry with value "1"
    struct key_value* new_entry = &collection->kvPairs[collection->count];
    new_entry->key = strdup(key);
    if (new_entry->key == NULL) return -1;
    new_entry->type = LIST_MAP_VALUE_STRING;
    new_entry->data.value = strdup("1");
    if (new_entry->data.value == NULL) {
        free(new_entry->key);
        return -1;
    }
    new_entry->expires_at_ms = 0;
    collection->count++;
    return 1;
}
        entry->data.list.items = NULL;
